...
# default class inside a finite bound, with no request reaching the delayed
# server.
for bad_timeout in nan inf 1e30; do
  reset_log
  start_fake --class-choice volume_cheap --class-confidence 0.97 --delay 30
  started=$(date +%s)
  run_class "$API_KEY" "$HOME_DIR" "$bad_timeout" "$TASK" -
  elapsed=$(( $(date +%s) - started ))
  reap_fake
  assert_typed_output "timeout $bad_timeout"
  assert_equals '"volume_cheap"' "$(json_get "$CLASS_OUT" class)" "timeout $bad_timeout keeps the default class"
  assert_equals '"api_error"' "$(json_get "$CLASS_OUT" flag)" "timeout $bad_timeout is flagged"
  assert_contains "$(json_get "$CLASS_OUT" reason)" 'fail_safe' "the $bad_timeout reason names the fail-safe"
...
# --- an unusable confidence floor is a usage error, never a silent rubric swap -
# A zero, negative, non-finite, or above-one threshold is refused before any
# request, because a NaN floor would make every comparison false and silently
# change the rubric.
for bad_threshold in nan inf 0 -0.1 1.1; do
  reset_log
  start_fake --class-choice volume_cheap --class-confidence 0.97
  CLASS_OUT=$(printf '%s' "$TASK" | TYPESAFE_API_KEY="$API_KEY" FM_HOME="$HOME_DIR" \
    FM_JV_CLASS_THRESHOLD="$bad_threshold" TYPESAFE_BASE_URL="$BASE" bash "$TOOL" - 2>"$STDERR_FILE")
  CLASS_RC=$?
  CLASS_ERR=$(cat "$STDERR_FILE")
  reap_fake
  expect_code 2 "$CLASS_RC" "threshold $bad_threshold is a usage error"
  assert_equals '' "$CLASS_OUT" "threshold $bad_threshold prints nothing on stdout"
  assert_contains "$CLASS_ERR" 'FM_JV_CLASS_THRESHOLD' "threshold $bad_threshold names the threshold"
  assert_absent "$LOG/body" "threshold $bad_threshold never reaches the network"
  pass "threshold $bad_threshold: usage error, no network"
done
