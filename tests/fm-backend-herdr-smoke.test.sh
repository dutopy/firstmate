...
pass "real herdr: container_ensure starts the isolated session's server, creates the System — Firstmate workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing System — Firstmate workspace, reports no seeded default tab on adoption)"
...
[ "$SM_LABEL_REAL" = "System — Secondmate smoketest-sm1" ] || fail "the secondmate workspace's real herdr label should be 'System — Secondmate smoketest-sm1', got '$SM_LABEL_REAL'"
pass "real herdr: the secondmate-shaped home's workspace is labeled '<lane> — Secondmate <secondmate-id>' in herdr itself"
...
POST_PRIMARY_ID=$(printf '%s' "$POST_LIST" | jq -r '.result.workspaces[]? | select(.label == "System — Firstmate") | .workspace_id')
POST_SM_ID=$(printf '%s' "$POST_LIST" | jq -r --arg l "System — Secondmate smoketest-sm1" '.result.workspaces[]? | select(.label == $l) | .workspace_id')
