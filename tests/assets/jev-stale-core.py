...
(default ``nan``) and whatever route ``FM_JV_STALE_ROUTE`` names. It never
touches the network.

The classifier tests point ``FM_JV_BLOCKER_CORE`` / ``FM_JV_LANE_CORE`` at this
file to prove that a stale core cannot push a non-finite or out-of-range
confidence past the wrapper boundary, independent of whatever the real shared
core currently does.
"""
...
def guard(unused_state, unused_criteria, **unused_kwargs):
    ...
    route = os.environ.get("FM_JV_STALE_ROUTE", "test_fixture_noise")
