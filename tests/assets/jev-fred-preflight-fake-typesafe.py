#!/usr/bin/env python3
"""Fake typesafe.ai System One endpoint for the Fred pre-flight tests.
...
Usage:
  python3 jev-fred-preflight-fake-typesafe.py --port-file <path> --log-dir <dir> \
      [--choice <optionId>] [--confidence <number>] [--status <http-code>] \
...
--confidence is parsed as a float, so the literal values nan and inf reach the
response body as the non-standard JSON constants NaN and Infinity; a test uses
that to prove a wrapper rejects a non-finite confidence instead of letting it
pass its own floor.
"""
...
def main() -> int:
    ...
    parser.add_argument("--choice", default="routine_reversible")
