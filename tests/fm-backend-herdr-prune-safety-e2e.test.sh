...
# "System — Firstmate" - with a live long-running process in that pane, exactly as
...
# Explicitly label the startup workspace "System — Firstmate" to create the collision
...
LIVE_CWD="$SCRATCH/firstmate-home"
mkdir -p "$LIVE_CWD"

fm_backend_herdr_server_ensure "$SESSION" || fail "could not start the isolated session's server"

CREATE_OUT=$(fm_backend_herdr_cli "$SESSION" workspace create --cwd "$LIVE_CWD" --label "System — Firstmate" --no-focus) \
...
[ "$LIVE_LABEL" = "System — Firstmate" ] || fail "the startup workspace label should be 'System — Firstmate', got '$LIVE_LABEL' - repro setup is wrong"
pass "repro setup: a pre-existing workspace labeled 'System — Firstmate' collides with the primary home's own label"
