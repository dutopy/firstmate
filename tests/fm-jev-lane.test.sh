...
# fail-safe ask_captain inside a finite bound, with no request reaching the
# delayed server.
for bad_timeout in nan inf 1e30; do
  reset_log
  start_fake --choice system --confidence 0.97 --delay 30
  started=$(date +%s)
  run_lane "$API_KEY" "$HOME_DIR" "$bad_timeout" "$REQUEST_PAYLOAD" -
  elapsed=$(( $(date +%s) - started ))
  reap_fake
  assert_typed_output "timeout $bad_timeout"
  assert_equals 'null' "$(json_get "$TOOL_OUT" route)" "timeout $bad_timeout assigns no lane"
  assert_equals '"ask_captain"' "$(json_get "$TOOL_OUT" flag)" "timeout $bad_timeout asks the captain"
