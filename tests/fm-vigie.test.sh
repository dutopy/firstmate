...
printf '%s\n' "$out" | jq -e '.schema == "fm-vigie.v1" and (.recommendations|length)==8 and .cadence == "daily"' >/dev/null \
...
printf '%s\n' "$out" | jq -e '.inventory.ready_prs.count == 2 and .inventory.client_gates.count == 1 and .inventory.credential_evidence.count == 1 and .inventory.pending_service_updates.count == 1 and ([.recommendations[].action] | index("decide:client-1"))' >/dev/null \
...
printf '%s\n' '{"recommendations":[{"key":"pr:pr-2"}]}' > "$TMP_ROOT/hidden-current.json"
hidden=$(FM_VIGIE_MAX=1 $VIGIE --json --event "$TMP_ROOT/hidden-current.json") || fail "bounded event digest failed"
printf '%s\n' "$hidden" | jq -e '.changes.resolved == [] and (.changes.new | index("pr:pr-1"))' >/dev/null \
  || fail "event deltas were computed after display truncation: $hidden"
UNKNOWN="$TMP_ROOT/unknown.sh"
cat > "$UNKNOWN" <<'SH'
#!/usr/bin/env bash
printf '%s\n' '{"schema":"fm-fleet-snapshot.v1","generated":"now","backlog":{"records":[]},"tasks":[],"secondmate_current":{"records":[]}}'
SH
chmod +x "$UNKNOWN"
