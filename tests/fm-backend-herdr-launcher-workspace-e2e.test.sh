...
# "System — Firstmate", and the previous label-first-match resolution put the worker in
...
[ "$(label_of_workspace "$WS_PRIMARY")" = "System — Firstmate" ] || fail "uniqA did not land in a 'System — Firstmate' workspace"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the spawn stole focus from the captain's workspace"
pass "real herdr E2E: with one 'System — Firstmate' workspace and no herdr parent, a crewmate still lands in this home's own workspace without stealing focus"
...
EOF
[ -n "$LAUNCH_PRIMARY_PANE" ] || fail "could not create a launcher pane inside the 'System — Firstmate' workspace"
...
[ "$(workspace_of_pane "$UNIQB_PANE")" = "$WS_PRIMARY" ] \
  || fail "a crewmate launched from the 'System — Firstmate' workspace must stay in it"
...
$(make_workspace "System — Firstmate")
EOF
[ -n "$WS_PRIMARY_DUP" ] || fail "could not create the second 'System — Firstmate' workspace"
[ "$WS_PRIMARY_DUP" != "$WS_PRIMARY" ] || fail "the two 'System — Firstmate' workspaces must be distinct"
DUP_COUNT=$(lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.label == "System — Firstmate")] | length')
[ "$DUP_COUNT" = 2 ] || fail "expected exactly two 'System — Firstmate' workspaces, got $DUP_COUNT"
...
[ "$DUPC_WS" = "$WS_PRIMARY_DUP" ] \
  || fail "a worker launched from the second 'System — Firstmate' workspace ($WS_PRIMARY_DUP) landed in '$DUPC_WS' instead"
...
pass "real herdr E2E: with two 'System — Firstmate' workspaces, a worker spawned from inside the second one lands in that exact workspace"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = "System — Firstmate" ] \
...
assert_contains_local "$(cat "$SPAWN_ERR")" "labeled 'System — Firstmate'" \
...
$(make_workspace "System — Secondmate $SM_ID")
EOF
...
[ "$(label_of_workspace "$SM2_WS")" = "System — Secondmate $SM2_ID" ] \
  || fail "a --secondmate launch should land in 'System — Secondmate $SM2_ID', got '$(label_of_workspace "$SM2_WS")'"
...
[ "$(label_of_workspace "$WS_PRIMARY_DUP")" = "System — Firstmate" ] || fail "teardown removed or renamed the launcher's workspace"
