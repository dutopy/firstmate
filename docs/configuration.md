...
`FM_HOME` determines Herdr's lane-qualified home label: the primary home uses `<Lane> — Firstmate`, and a secondmate home marked by `.fm-secondmate-home` uses `<Lane> — Secondmate <secondmate-id>`.
`<Lane>` is read from local gitignored `config/herdr-lane` (whitespace-stripped, case-preserved, default `System`); the file is per-home and deliberately not inheritable, because each home owns the lane it serves.
[`herdr-backend.md`](herdr-backend.md#watching-and-task-containers) owns the accepted lane values and warning fallback, launcher-bound workspace placement, the label-only fallback, collision handling, and recovery behavior.
...
`bin/fm-procevent-hermes-decisions.sh` is the built-in answer source for a Hermes pilot profile's `#decisions` channel: the profile's own `decisions-register` plugin captures a keyed captain answer into one durable record, this adapter reads that record through the profile's own CLI and prints the keyed line the one keyed-answer intake reads, and its `autohandle` acknowledges the record in the profile store without acknowledging the result to the runner, so the handler's wake is never suppressed.
Its header owns the commands, the profile flags, and the silent no-result and loud-failure wire contract.
...
FM_PI_TURNEND_FOLLOWUP_CEILING=12   # consecutive guard-driven follow-ups a Pi primary session may take before the harness-side bound stops emitting them entirely and records stopped=ceiling in state/.turnend-pi-followups; reset by a healthy/not-needed guard verdict, a real captain message, or a fresh session
FM_TURNEND_GUARD_ALERT_AT=3   # consecutive guard-driven follow-ups a passive adapter reports via FM_TURNEND_FOLLOWUP_COUNT at which bin/fm-turnend-guard.sh prints its ONE deduplicated early alert
FM_TURNEND_GUARD_NOTICE_AT=6   # same count at which the shared guard prints its ONE final loud notice naming bin/fm-turnend-guard-clear.sh; clamped up to FM_TURNEND_GUARD_ALERT_AT when set below it
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; this only makes the repetition louder, it does not bound it
FM_WEDGE_ESCALATE_MAX=12   # consecutive wedge escalations on one unchanged pane state before the ladder stops, reports why once, and absorbs every later threshold crossing (bin/fm-watch.sh's wedge_ceiling_record); an idle pane whose task record is still open in this home is not a wedge the ladder can resolve by climbing, and every genuine reset of the window's pane/hash state clears the count with its alert and ceiling markers
