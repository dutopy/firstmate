...
FM_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=14  # days after which an undated captain hold (no hold-until; age from its UTC hold-set timestamp using floored elapsed days, falling back to since for legacy unstamped holds) is projected as a Charted Next gate instead of a live Captain's Call; 0 ages every such dated hold
