...
- Any API or network error, a malformed or out-of-vocabulary response, an unusable `FM_JV_CLASS_TIMEOUT` value (non-finite, negative, or above the ceiling), a host that cannot arm the bound, the wall-clock bound itself, and a missing or rejected API key resolve to the default class and its mapped effort, with `flag` set to `api_error` and exit 0.
...
Exit 2 is reserved for a usage or environment error: an unreadable or empty task description, an unknown class or effort value, a bad threshold (a `FM_JV_CLASS_THRESHOLD` that is not a finite number in `(0, 1]`) or default class, missing `python3`, an unreadable shared core, or an unreadable or malformed canonical dispatch config on the profile-lookup path.
It prints nothing on stdout and makes no network call.
A non-finite or zero confidence floor is refused here rather than silently changing the rubric, because a NaN floor would make every comparison false.
