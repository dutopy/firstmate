...
Suspicion-triggered anomaly reporting uses `bin/fm-reflex.sh` as a bounded command surface, not a daemon or parallel ledger. Its `intake`, `report`, `review`, and `resolve` commands append privacy-safe lifecycle events to the task's existing status log; raw evidence is replaced by a SHA-256 digest, `cause=unknown` remains explicit when no cause is established, and the existing watcher/status classifier surfaces intake and report events. Deduplication is keyed to the normalized task, instance, cause, and evidence digest, while an explicit `needs-decision` review event and ordinary `resolved` event preserve intake/report/review separation and native open-decision semantics.
