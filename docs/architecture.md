...
The dead-record bound is not the ladder's only ceiling.
A pane whose task record is still open in this home and whose agent is simply idle is not a wedge the ladder can resolve by climbing, so the consecutive-escalation count is capped at `FM_WEDGE_ESCALATE_MAX` (default 12): past that ceiling `bin/fm-watch.sh`'s `wedge_ceiling_record` reports the stop ONCE per unchanged pane state, naming the count, the label that started the timer, and the fact that the pane is idle with its task record still open, then absorbs every later threshold crossing without escalating.
`FM_WEDGE_DEMAND_INSPECT_COUNT` only makes the repetition louder, so it never bounded it; the ceiling is what does.
The crossing of that small threshold is also the event-driven early detector: `wedge_repeat_alert` queues ONE durable `check:` wake keyed on the pane, naming the repeating source, so a looping pane is heard before it can saturate the session, and the episode marker bounds it to one alert per pane state rather than an alert loop of its own.
Every genuine reset of a window's pane or hash state clears the count, the alert marker, and the ceiling record together (`clear_wedge_tracking`).
...
Because a pooled slot is reused, one slot can carry more than one task record; [`bin/fm-teardown.sh`](../bin/fm-teardown.sh) owns the record-exclusivity refusal and the reconciliation that returns a fully settled slot once, and [treehouse slot reconciliation verification](verification/treehouse-slot-reconciliation.md) owns the active evidence.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=`, any available `pr_head=`, and the recorded patch identity through `bin/fm-pr-check.sh` before calling the forge CLI.
The recorded patch identity is the ready report's promise that the patch it stood for is the patch that merges: the merge path recomputes the forge's live base-to-head patch identity and refuses when it differs, so a force-push that changes the patch is re-validated and recorded again rather than landing on green checks that only prove a different patch.
A rebase that changes no patch content keeps that identity, so an ordinary rebase still merges.
...
`--auto`, `--admin`, and branch-deletion flags are refused unless `--attended-override` is passed for an explicit captain instruction; that override never skips the live green check, the away-grant check, a captain hold, or the patch-identity comparison.
