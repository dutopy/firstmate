...
| `fm-axi-status.sh`       | Read live aggregate AXI status, project six bounded error/log/hook targets, validate its journal, or idempotently write a structured event ([axi-status.md](axi-status.md)) |
...
| `fm-jev-lane.sh`         | Advisory, fail-safe lane classification of one request over the shared Jev core ([jev-guard.md](jev-guard.md)) |
...
| `fm-pr-check.sh`         | Record validated `pr=`, any available `pr_head=`, and the ready report's patch identity, then atomically arm a static merge poll |
...
| `fm-discord-conversation-console.sh` | Talk to Firstmate from Discord: capture #firstmate captain messages, transcribe captain voice messages through Groq Whisper, post action cards whose buttons record the captain's answer, and answer in the same conversation thread ([discord-conversation-console.md](discord-conversation-console.md)) |
