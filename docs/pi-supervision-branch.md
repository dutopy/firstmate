...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  A needs-decision - a product call, a destructive-action confirmation, a captain-hold, a secondmate escalation, or any other decision the captain must answer - gets the identical treatment whether surfaced as a signal, a captain-held stale recheck, or a heartbeat backstop. `bin/fm-watch.sh` marks that row's queued payload `needs-decision:`, `scopeForUnreadWake` excludes it from what the branch may claim by that payload marker, and the dispatcher forces its own triggering close to main by cross-referencing the unchanged signal, stale, or heartbeat wake message against the excluded keys. Every needs-decision reaches the captain directly, without the supervision-branch hop, exactly like a check-kind close.
...
`tests/fm-watch-triage.test.sh` covers `bin/fm-watch.sh`'s side of the contract end to end: needs-decision signal, captain-held stale, and heartbeat rows are marked `needs-decision:`, rejected reserved-key transitions are still marked, and routine captain-relevant payloads stay unmarked.
