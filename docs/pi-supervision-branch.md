...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  A needs-decision - a product call, a destructive-action confirmation, a captain-hold, a secondmate escalation, or any other decision the captain must answer - gets the identical treatment whether surfaced as a signal, any stale path, or a heartbeat backstop.
  `bin/fm-watch.sh` marks decision-bearing signal and heartbeat rows with a `needs-decision:` queue payload, while `fm_wake_append_stale` in `bin/fm-wake-lib.sh` applies that marker when the task-status classifier reports an actionable needs-decision or an active captain-held transfer, including Herdr push-transition escalation.
  `scopeForUnreadWake` excludes marked rows from what the branch may claim, and the dispatcher forces each marked row's own triggering close to main by cross-referencing the unchanged signal, stale, or heartbeat wake message against the excluded keys.
  Every needs-decision reaches the captain directly, without the supervision-branch hop, exactly like a check-kind close.
  ...
  The identical non-veto guarantee applies to a co-present needs-decision row.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every branch-ownable unread row is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
...
A heartbeat is never vetoed or ridden into main by a co-present main-owned row.
...
Decision-bearing signal, stale, and heartbeat rows follow the same non-veto rule and direct-main path defined in [Components and their owners](#components-and-their-owners).
...
The branch-offer, heartbeat-offer, heartbeat-not-ridden-by-a-check, main-only-check-class, and needs-decision-main-routing tests remain in `tests/fm-pi-watch-extension.test.sh`, the recovery test remains in `tests/fm-session-start.test.sh`, and the per-actor consume regression remains in `tests/fm-wake-queue.test.sh`.
`tests/fm-watch-triage.test.sh` covers `bin/fm-watch.sh`'s side of the contract end to end: needs-decision signal, decision-bearing and captain-held stale, and heartbeat rows are marked `needs-decision:`, rejected reserved-key transitions are still marked, and routine captain-relevant payloads stay unmarked.
`tests/fm-supervision-events.test.sh` covers the shared stale appender and the Herdr push-transition path.
