...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  A needs-decision status append - a product call, a destructive-action confirmation, a captain-hold, a secondmate escalation, or any other `needs-decision:` event a crewmate appends to its status log - gets the identical treatment even though it keeps the ordinary `signal` kind: `bin/fm-watch.sh`'s `signal_files_actionable` marks that row's queued payload `needs-decision:` instead of the ordinary `signal:`, `scopeForUnreadWake` excludes it from what the branch may claim by that payload marker, and the dispatcher forces its own triggering close to main by cross-referencing the trigger's file list against the excluded keys - never by changing the wake message text itself, so every other harness-arm script keeps seeing the same `signal:<files>` shape it always has. Every needs-decision reaches the captain directly, without the supervision-branch hop, exactly like a check-kind close.
  ...
  A co-present main-owned check row no longer defers that review to main, because it is not fleet context the branch is missing and main is woken for it on its own triggering close. The identical non-veto guarantee applies to a co-present needs-decision row.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every non-check, non-needs-decision row in the unread queue is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
...
The branch-offer, heartbeat-offer, heartbeat-not-ridden-by-a-check, main-only-check-class, and needs-decision-signal-stays-on-main tests remain in `tests/fm-pi-watch-extension.test.sh` (the last exercising `offerWakeToBranch`'s file-list cross-reference end to end), the recovery test remains in `tests/fm-session-start.test.sh`, and the per-actor consume regression remains in `tests/fm-wake-queue.test.sh`.
`tests/fm-watch-triage.test.sh` covers `bin/fm-watch.sh`'s side of the contract end to end: a needs-decision signal row's queued payload is marked `needs-decision:`, a needs-decision whose key transition was rejected by the reserved-key vocabulary (fm-classify-lib.sh's "reconciliation-required: " wrapper) is still marked, and a routine captain-relevant signal's payload stays unmarked.
