...
export default function (pi: ExtensionAPI) {
  ...
  function offerWakeToBranch(message: string): Promise<void> | null {
    ...
    // A needs-decision signal, stale recheck, or heartbeat backstop gets the
    // identical main-only treatment as a check-kind trigger: every
    // needs-decision must reach the captain directly, never taking the
    // supervision-branch hop first (docs/pi-supervision-branch.md "Autonomy").
    // The wake message text stays unchanged; this cycle's trigger key is
    // cross-referenced against the keys scopeForUnreadWake excluded for a
    ...
    const triggerKeys = /^signal:/.test(message)
      ...
      : /^stale:\s*(\S+)/.exec(message)?.slice(1) ?? (/^heartbeat($|:)/.test(message) ? ["heartbeat"] : []);
