...
# Primary config/discord-session-mirror.json is a captain-approved Discord
# session-mirror configuration, so a secondmate home carries the same mirror
# target and its own sync run can populate the client guild forums.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode discord-session-mirror.json}"
