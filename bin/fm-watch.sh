...
procevent_surface_queued() {
  local key reason payload captured="" stranded="" unstarted=""
  PROCEVENT_SURFACED=
  INBOX_SURFACED=
  ...
  reason=""
  if [ -n "$captured" ]; then
    reason="check: process-event result captured:$captured"
  fi
  if [ -n "$stranded" ]; then
    if [ -n "$reason" ]; then reason="$reason;"; else reason="check:"; fi
